  ...
  Future<void> initiatePayment({
    ...
    try {
      ...
      if (isAppCheckFailure) {
        ...
        // Server detected a price change — surface the updated price to the user
        // so they can review and confirm before retrying.
        final details = e.details;
        if (details is Map && details['code'] == 'PRICE_CHANGED') {
          final latest = details['latestAmount'];
          final prev = details['previousAmount'];
          onFailure(
            'The service price changed from ₹$prev to ₹$latest. '
            'Please go back and confirm the updated price before paying.',
          );
        } else {
          onFailure(e.message ?? 'Payment could not be initiated. Please try again.');
        }
